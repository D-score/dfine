...
#' @noRd
calculate_fit_statistics <- function(data,
                                     items,
                                     fit,
                                     itembank,
                                     ability,
  ...
  if (!"equate" %in% colnames(itembank)) itembank$equate <- NA

  # check if items are in data
  visit_var    <- if (is.null(fit$visit_var)) c("subjid", "agedays") else fit$visit_var
  item_var     <- if (is.null(fit$item_var)) "item" else fit$item_var
  response_var <- if (is.null(fit$response_var)) "response" else fit$response_var
  ...
  if (fit$shape == "wide") {
    residuals <- data |>
      ...
      pivot_longer(names_to = item_var, values_to = response_var, cols = -c(visit_var, ability_var), values_drop_na = TRUE)
  ...
  residuals <- residuals |>
    ...
    select(one_of(c(visit_var, "equate", item_var, response_var, "tau", ability_var))) |>
    drop_na(all_of(ability_var)) |>
    mutate(taut = (.data$tau - transform[1L]) / transform[2],
  ...
  residuals <- residuals |>
    mutate(w = pmax(.data$p^2 * (1 - .data$p) + (1 - .data$p)^2 * .data$p, 0.01),
  ...
  if (is.null(fit$equate)) equate_fit <- NULL
