...
#
#  C. Select SF/LF items with at least 10 observation in both categories
#
...
valid_items <- responses |>
  ...
  pull(item)
items <- intersect(c(items_sf, items_lf), valid_items)
responses <- responses |>
  filter(item %in% items)

# table(responses$vist_type, responses$country)
#
...
path <- file.path("~/project/gsed/phase2/sf")
device <- "pdf"
...
path <- file.path("~/project/gsed/phase2/lf")
device <- "pdf"
if (!is.null(file) & device == "pdf") {
